...
> From a proof of `Implies p q` and a proof of `p`, we obtain a proof of
> `q`.
...
> Suppose that, assuming `p` as a hypothesis, we have a proof of `q`. Then 
> we can "cancel" the hypothesis and obtain a proof of `Implies p q`.
...
and proofs. Determining that an expression `t` is a correct proof of 
assertion `p` would then simply be a matter of checking that `t` has type 
`Proof p`.
...
We will see in [Chapter Structures and Records](./structures_and_records.md) 
that certain types in Lean are *structures*, which is to say, the type is 
defined with a single canonical *constructor* which builds an element of the
